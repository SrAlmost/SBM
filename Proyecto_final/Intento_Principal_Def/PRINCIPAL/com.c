...
//char recibidoRX[256]; //caracter recibido: INICIO|COMANDO|LONGITUD|FIN
...
//static void procesarDatosRecibidos( char *recibido );
...
__NO_RETURN static void ThcomRX (void *argument){
	...
	while(1){
		...
		osThreadFlagsWait(0x10, NULL, osWaitForever);
		iteHora = 0;
		iteTemp = 0;
		if(cmd == INI && receive == ESPERANDO_INICIO)
		{
			receive = COMANDO;
			do
			{
				...
				switch(receive)
				{
					case ESPERANDO_INICIO:
						break;
					case COMANDO:
						if(cmd == HORA || cmd == TEMPERATURA || cmd == MEDIDAS || cmd == BORRAR)
						{
							comandoRX = cmd;
							receive = LONGITUD;
						}else
			...
			}while(cmd != FIN || receive != ESPERANDO_INICIO);
		}
