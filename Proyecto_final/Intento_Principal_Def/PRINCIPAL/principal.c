...
static void programacion(){
	...
	switch(recived.c)
	{
		...
		case MEDIDAS:
			for(int i = 0; i < 10; i++)
			{
        if(strcmp(medidasBuffer[i], "") != 0)
        {
          sscanf( medidasBuffer[i], "%d:%d:%d--Tm:%f--Tr:%f--D:%d%%", &hora, &min, &seg, &recived.temperatura, &recived.temperaturaREF, &PWM);
					recived.hora = hora;
					recived.minutos = min;
					recived.segundos = seg;
					recived.PWM = PWM;
          osMessageQueuePut(mid_MsgQueuePCTX, &recived, 0U, 0U);
        }
      }
		...
		switch(pulsacion.direccion)
		{
			...
			case IZQUIERDA:
				if(varMod != 0x80)
				{
					...
					if(varMod == 0x40 || varMod == 0x10 || varMod == 0x01)
					{
						pos = pos - 8;
					}else if(varMod == 0x04)
					{
			...
			case ARRIBA:
				switch(varMod)
				{
					...
					case 0x04:
						...
						break;
					case 0x02:
						temRefMod = (temRefMod > 20.0) ? temRefMod : (temRefMod + 10);
						break;
					case 0x01:
						temRefMod = (temRefMod >= 29.5) ? temRefMod : (temRefMod + 1);
						break;
					case 0x0A:
						temRefMod = (temRefMod == 30.0) ? temRefMod : (temRefMod + 0.5);
						break;
				}
				pulsacion.direccion = 0x00;
				break;
			case ABAJO:
				switch(varMod)
				{
					...
					case 0x04:
						...
						break;
					case 0x02:
						temRefMod = (temRefMod < 15.0) ? temRefMod : (temRefMod - 10);
						break;
					case 0x01:
						temRefMod = (temRefMod <= 5.5) ? temRefMod : (temRefMod - 1);
						break;
					case 0x0A:
						temRefMod = (temRefMod == 5.0) ? temRefMod : (temRefMod - 0.5);
						break;
				}
				pulsacion.direccion = 0x00;
				break;
