...
static ADC_HandleTypeDef adchandle; //Manejador ADC
	/*------------------- HILO POTENCIOMETROS -------------------*/
static osThreadId_t tid_Thpot; //id del hilo que gestiona la temperatura 
...
int8_t Init_Thpot (void) {
	...
	ADC_pins_config();
	//Configuramos el ADC
	if( ADC_Init_Single_Conversion(&adchandle , ADC1) != 0){		return -1;	}
	//Creamos el hilo de los potenciometros
  tid_Thpot = osThreadNew(Thpot, NULL, NULL);
  if (tid_Thpot == NULL) {    return(-1);  }
  return(0);
}
	/*------------------- HILO POTENCIÓMETROS -------------------*/
__NO_RETURN static void Thpot(void *argument) {
	while (1) {
		...
		medidas.temperaturaREF = conversionTemperatura( ADC_getVoltage() );
		//Medimos la temperatura por medio del canal 3
			//Configuramos el canal 3
		ADC_Channel_Config(3,1);
			//Medimos la temperatura
		medidas.temperaturaMED = conversionTemperatura( ADC_getVoltage() );
